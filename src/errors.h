   ...
   public:
    PageNotExistError(const std::string &table_name, int page_no)
        : RMDBError("Page " + std::to_string(page_no) + " in table " + table_name + "not exits") {
            std::cout << "Error: " << "Page " + std::to_string(page_no) + " in table " + table_name + "not exits" << std::endl;
        }
};

class InvalidOperatorError : public RMDBError {
    public:
    InvalidOperatorError()
        : RMDBError("Found Invalid Operator!"){}
};
