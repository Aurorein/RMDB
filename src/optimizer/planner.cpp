...
std::pair<int, IndexMeta> Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
// bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    ...
    return tab.have_index(index_col_names);
    // if(tab.have_index(index_col_names)) return true;
    // return false;
}
...
std::shared_ptr<Query> Planner::logical_optimization(std::shared_ptr<Query> query, Context *context)
{
    ...
    // 等价类替换
    auto &conds = query.get()->conds;
    for(auto cond_iter = conds.begin();cond_iter != conds.end();++cond_iter){
        auto &cond = *cond_iter; 
        // 发现类似w_id = 1的情况
        if(cond.op == OP_EQ && cond.is_rhs_val) {
            auto it = conds.begin();
            for(; it != conds.end(); it++) {
                if(it->is_rhs_val == false && it->rhs_col == cond.lhs_col) {
                    it->rhs_val = cond.rhs_val;
                    it->is_rhs_val = true;
                }else if(it->is_rhs_val == false && it->lhs_col == cond.lhs_col) {
                    it->rhs_val = cond.rhs_val;
                    // 交换左右
                    it->lhs_col = it->rhs_col;
                    it->is_rhs_val = true;
        ...
        //             if((*it).lhs_col.col_name.compare(cond.rhs_col.col_name) == 0 && (*it).lhs_col.tab_name.compare(cond.rhs_col.tab_name) == 0){
        //                 equi_cond.lhs_col.tab_name = cond.lhs_col.tab_name;
        //                 equi_cond.lhs_col.col_name = cond.lhs_col.col_name;
        //                 equi_cond.is_rhs_val = true;
        //                 equi_cond.op = OP_EQ;
        //                 equi_cond.rhs_val = (*it).rhs_val;

        //                 break;

        //             }else if((*it).lhs_col.col_name.compare(cond.lhs_col.col_name) == 0 && (*it).lhs_col.tab_name.compare(cond.lhs_col.tab_name) == 0){
        //                 equi_cond.lhs_col.tab_name = cond.rhs_col.tab_name;
        //                 equi_cond.lhs_col.col_name = cond.rhs_col.col_name;
        //                 equi_cond.is_rhs_val = true;
        //                 equi_cond.op = OP_EQ;
        //                 equi_cond.rhs_val = (*it).rhs_val;
        ...
        //     if(it != conds.end()){
        //         conds.erase(cond_iter);
        //         conds.emplace_back(equi_cond);
        //     }
        // }
    }
...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query> query)
{
    ...
    for (size_t i = 0; i < tables.size(); i++) {
        ...
        auto [index_mode, index_meta] = get_index_cols(tables[i], curr_conds, index_col_names);
        if (index_mode == -1) {  // 该表没有索引
            index_col_names.clear();
            table_scan_executors[i] = 
                std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, tables[i], curr_conds, index_col_names);
        } else if(index_mode == 0) {  // 存在索引
            auto index_scan = std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, tables[i], curr_conds, index_col_names);
            // 将找到的最匹配的索引赋给index_scan plan
            index_scan->index_meta_ = index_meta;
            table_scan_executors[i] = index_scan;
        } else if(index_mode == 1) {
            auto index_scan = std::make_shared<ScanPlan>(T_IndexModeOneScan, sm_manager_, tables[i], curr_conds, index_col_names);
            // 将找到的最匹配的索引赋给index_scan plan
            index_scan->index_meta_ = index_meta;
            table_scan_executors[i] = index_scan;
        }
    ...
    for(auto order : x->orders) {
        ...
        for(auto &col : all_cols) {
            if(col.name.compare(order->cols->col_name) == 0) {
                order_col.tabcol = {.tab_name = col.tab_name, .col_name = col.name};
                order_col.is_desc = (order->orderby_dir == ast::OrderBy_DESC);
            }
        }
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query, Context *context)
{
    ...
    if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
        ...
        // load;
        auto load_plan = std::make_shared<DMLPlan>(T_LoadData, std::shared_ptr<Plan>(),  x->tab_name,  
                                                    std::vector<Value>(), std::vector<Condition>(), std::vector<SetClause>());
        load_plan->file_path_ = x->file_path;
        plannerRoot = load_plan;
        ...
        auto [index_mode, index_meta] = get_index_cols(x->tab_name, query->conds, index_col_names);
        if (index_mode == -1) {  // 该表没有索引
            index_col_names.clear();
            ...
            auto index_scan = std::make_shared<ScanPlan>(T_IndexModeOneScan, sm_manager_, x->tab_name, query->conds, index_col_names);
            // 将找到的最匹配的索引赋给index_scan plan
            index_scan->index_meta_ = index_meta;
            table_scan_executors = index_scan;
        }
        ...
        if (index_mode == -1) {  // 该表没有索引
            ...
            // // 优化点：如果是MAX或者MIN算子，并且发现用索引扫描，那么可以用
            // if(query->aggre_meta.op_ == AggreOp::AG_MIN || query->aggre_meta.op_ == AggreOp::AG_MAX) {
            //     // 并且聚合的列刚好在索引里
            //     const auto &index_cols = index_meta.cols;
            //     if(index_cols.size() > query->conds.size()) {
                
            //         size_t i = 0;
            //         for(i = 0; i < query->conds.size() && i < index_cols.size(); i++) {
            //             if(query->conds[i].lhs_col.col_name == index_cols[i].name) {
            //                 continue;
            //             }else {
            //                 break;
            //             }
            //         }
            //         if(i == query->conds.size() && )
            // }
