...
void SetTransaction(txn_id_t *txn_id, Context *context) {
    ...
    // 2. 如果事务已经commit或者abort了，则需要从txn map中删除
    if(context->txn_->get_state() == TransactionState::COMMITTED ||
        context->txn_->get_state() == TransactionState::ABORTED) {
        
        // 先释放之前事务占用的内存
        txn_manager->delete_transaction(*txn_id);
...
void *client_handler(void *sock_fd) {
    ...
    while (true) {
        ...
        context->outputFile_ = &OutputFile;
        ...
        std::cout << "Time taken for sql: " << duration.count() / 1000000.0 << " s" << std::endl << std::endl;
    }
