...
public:
    ...
    DeleteLogRecord(txn_id_t txn_id, RmRecord& delete_value, Rid& rid, std::string table_name) 
        ...
        rid_ = rid;
        // 注意：这个int存的不是txn_id，而是delete_value_.size
        log_tot_len_ += sizeof(int);
    ...
    // 增加析构函数，释放table_name_的内存，但是有个问题，不能保证table_name_的内存一定可以释放，万一table_name = nullptr?
    ~DeleteLogRecord() {
        delete [] table_name_;
    }
...
public:
    ...
    UpdateLogRecord(txn_id_t txn_id, RmRecord& before_value,RmRecord& after_value, Rid& rid, std::string table_name) 
        ...
        rid_ = rid;
        // 注意：这个int存的不是txn_id，而是update_value_.size
        log_tot_len_ += sizeof(int);
    ...
    // 因为要同时支持undo和redo操作，所以需要before_value_和after_value_
    RmRecord before_value_;     // update前的值
    RmRecord after_value_;      // update后的值
    Rid rid_;                   // 更新记录的位置
...
/**
 * TODO: create table操作的日志记录
*/
class CreateTableRecord: public LogRecord {
public:
    CreateTableRecord() {
        log_type_ = LogType::CREATE_TABLE;
        lsn_ = INVALID_LSN;
        log_tot_len_ = LOG_HEADER_SIZE;
        log_tid_ = INVALID_TXN_ID;
        prev_lsn_ = INVALID_LSN;
    ...
    CreateTableRecord(txn_id_t txn_id, const std::vector<ColDef>& col_defs, std::string table_name) 
        : CreateTableRecord() {
        log_tid_ = txn_id;
        log_tot_len_ += sizeof(int);
        // 对col_defs_进行手动赋值，将传入的col_defs向量中的每个元素复制到col_defs_向量中
        for (auto &col_def : col_defs) {
            col_defs_.push_back(col_def);
            log_tot_len_ += sizeof(col_def);
        }
    ...
    // 把create table日志记录序列化到dest中
    void serialize(char* dest) const override {
        LogRecord::serialize(dest);
        int offset = OFFSET_LOG_DATA;
        int size = col_defs_.size();
        memcpy(dest + offset, &size, sizeof(int));
        offset += sizeof(int);
        for(auto &col_def : col_defs_){
            memcpy(dest + offset, &col_def.len, sizeof(int));
            offset += sizeof(int);
            memcpy(dest + offset, col_def.name.c_str(), col_def.len);
            offset += col_def.len;
            memcpy(dest + offset, &col_def.type, sizeof(col_def.type));
            offset += sizeof(col_def.type);
        }
        memcpy(dest + offset, &table_name_size_, sizeof(size_t));
        offset += sizeof(size_t);
        memcpy(dest + offset, table_name_, table_name_size_);
    }
    // 从src中反序列化出一条create table日志记录
    void deserialize(const char* src) override {
        LogRecord::deserialize(src);
        int offset = OFFSET_LOG_DATA;
        int size = *reinterpret_cast<const int*>(src + offset);
        offset = OFFSET_LOG_DATA + sizeof(int);
        for(int i = 0;i< size;++i){
            ColDef col;
            col.len = *reinterpret_cast<const int*>(src + offset);
            offset += sizeof(int);
            col.name = std::string(src + offset, col.len);
            offset += col.len;
            col.type = *reinterpret_cast<const ColType*>(src + offset);
            offset += sizeof(ColType);
            col_defs_.push_back(col);
        }
        table_name_size_ = *reinterpret_cast<const size_t*>(src + offset);
        offset += sizeof(size_t);
        table_name_ = new char[table_name_size_];
    ...
    std::vector<ColDef> col_defs_;    // 要插入的col_defs
    char* table_name_;          // 更新记录的表名称
    size_t table_name_size_;    // 表名称的大小
};
...
public:
    ...
    lsn_t flush_log_to_disk(LogRecord* log_record);
