...
bool LockManager::check_conflict(LockMode want_mode, LockMode curr_mode) {
    switch(want_mode) {
        case LockMode::INTENTION_SHARED: {
            return curr_mode == LockMode::EXLUCSIVE;
        }
        case LockMode::INTENTION_EXCLUSIVE: {
            return (curr_mode == LockMode::SHARED) | (curr_mode == LockMode::EXLUCSIVE);
        }
        case LockMode::SHARED: {
            return (curr_mode == LockMode::INTENTION_EXCLUSIVE) | (curr_mode == LockMode::EXLUCSIVE);
        }
        case LockMode::EXLUCSIVE: {
            return true;
        }
        case LockMode::S_IX: {
            return curr_mode != LockMode::INTENTION_SHARED;
        }
        default:
        break;
    }
...
bool LockManager::deadlock_prevention_check(LockRequestQueue *request_queue, Transaction *txn, LockMode want_mode) {
    // if deadlock, return true
    for(auto &req : request_queue->request_queue_) {
        if(check_conflict(want_mode, req.lock_mode_)){
...
=======
// no-wait定时
bool LockManager::lock_shared_on_record_wait_time(Transaction* txn, const Rid& rid, int tab_fd) {
    for(int i = 0; i < RESTART_COUNT; i++) {
        if(lock_shared_on_record(txn, rid, tab_fd)) {
            // std::cout << "Try " << i << " times for lock on record " << tab_fd << " (" << rid.page_no << ", " << rid.slot_no << ")" << std::endl;  
            return true;
        }
...
bool LockManager::lock_exclusive_on_record_wait_time(Transaction* txn, const Rid& rid, int tab_fd) {
    for(int i = 0; i < RESTART_COUNT; i++) {
...
bool LockManager::lock_shared_on_table_wait_time(Transaction* txn, int tab_fd) {
    for(int i = 0; i < RESTART_COUNT; i++) {
        if(lock_shared_on_table(txn, tab_fd)) {
            // std::cout << "Try " << i << " times for lock on table " << tab_fd << std::endl;  
            return true;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(INTERVAL_TIME));
    }
...
bool LockManager::lock_exclusive_on_table_wait_time(Transaction* txn, int tab_fd) {
    for(int i = 0; i < RESTART_COUNT; i++) {
...
bool LockManager::lock_IS_on_table_wait_time(Transaction* txn, int tab_fd) {
    for(int i = 0; i < RESTART_COUNT; i++) {
...
bool LockManager::lock_IX_on_table_wait_time(Transaction* txn, int tab_fd) {
    for(int i = 0; i < RESTART_COUNT; i++) {
